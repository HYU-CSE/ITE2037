 ...
 * 
 */
public class Player_HONG extends Player {
	...
	public Player_HONG() {
		name = "HONG"; // TODO 자신이 만들 플레이어의 이름으로 name 필드를 초기화하세요.
	...
	void SetBasePoint() {
		basePoint = new Point(Constants.Classroom_Width / 2,
				Constants.Classroom_Height / 2);
	}
	...
	public DirectionCode Survivor_Move() {
		...
		if (this.gameInfo.GetCurrentTurnNumber() <= 10) {
			/*
		...
		else if (this.gameInfo.GetCurrentTurnNumber() <= 70) {
			/*
			 * 외톨이의 생존자 이동: 다른 플레이어가 가장 적은 방향을 선택
			 */
	...
	public DirectionCode Infected_Move() {
		...
		for (PlayerInfo other : othersInfo_withinSight) {
			...
			if ((other.GetState() == PlayerInfo.State.Survivor || other
					.GetState() == PlayerInfo.State.Corpse)
					&& v.GetDistance() > 1) {
				if (v.y_offset < 0)
					++weights[0];
	...
	public Point Soul_Spawn() {
		...
		if (this.gameInfo.GetCurrentTurnNumber() <= 80)
			return basePoint;
