 ...
 *
 */
public class Player_joonwook extends Player
{
	/**
	 ...
	 */
	public Player_joonwook()
	{
		name = String.format("외톨이");
	...
	public DirectionCode Survivor_Move()
	{
		...
		if ( gameInfo.GetCurrentTurnNumber() <= 40 )
		{/*
		...
		for ( PlayerInfo other : othersInfo_detected )
		{
			...
			else if(other.GetState()== PlayerInfo.State.Corpse)
			{
				if ( v.y_offset < 0 )
					++others[0];
		...
		else if(gameInfo.GetCurrentTurnNumber() >= 40)
		{
			/*
			 * 정찰병의 생존자 이동: 생존자 수 x 시체와 감염체 수가 가장 큰 방향을 선택
			 */
	...
	public DirectionCode Infected_Move()
	{
		...
		else if(gameInfo.GetCurrentTurnNumber()>40 &&gameInfo.GetCurrentTurnNumber() <80 )
		{
			/*
