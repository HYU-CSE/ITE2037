...
public class Player_2008027687 extends Player {
	...
	public Player_2008027687() {
		name = "2008027687"; // TODO 자신이 만들 플레이어의 이름으로 name 필드를 초기화하세요.
		acceptDirectInfection = false; // TODO '직접 감염'을 받으려는 경우 이 필드를 true로 두고
										// 아닌 경우 false로 두세요.
		this.receiveOthersInfo_detected = true; // 포착 정보를 수신
	}

	@Override
	public DirectionCode Survivor_Move() {
		// TODO 생존자 상태일 때 이동하기 위한 생각을 여기에 담으세요.
		// 인간+감염체 많은 곳으로 이동 for 많이 관측하기 위해 && 감염체 많은 곳으로 이동하기 위해
	...
	public DirectionCode Infected_Move() {
		...
		// 3번만에 갈 수 있는 시체가 있다면, 가장 가까운 시체가 있는 곳으로 간다.
		// 그게 아니라면 약 10턴간 인간 많은 곳을 찾아 헤맨다. 감염시킨다.
		// 정화기도 1턴.
		plusInfectedTurnNumber();

		/*
		 * 포식자의 감염체 이동: 현 위치에 시체가 있는 경우 제자리에 머무름. 그렇지 않은 경우 생존자 및 시체가 가장 많은 방향으로
		 * 이동.
		 */
		...
		// 만약 감염체 상태로 5턴이 넘고 체력이 30보다 작으면 정화기도 해서 빨리 상태 탈출 -> 시체가 없는 곳으로 이동
	...
	/**
	 * '방향 섞기' 기능을 사용하기 위해 방향 목록을 초기화합니다. 이 메서드는 첫 턴의 Soul_Stay()에서 호출되어야 합니다.
	 */
