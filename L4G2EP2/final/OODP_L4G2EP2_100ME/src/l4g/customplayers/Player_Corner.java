...
public class Player_Corner extends Player
{
   ...
   void UpdateState()
   {
	   switch(state)
	      {
	         case GoRight:
	            if(this.myInfo.position.column == Constants.Classroom_Width-11)//교실 가로-X칸
	               state = GoDown;
	            break;
	         case GoDown:
	            if(this.myInfo.position.row == Constants.Classroom_Height-11)//교실 세로 -X칸
	               state = GoLeft;
	            break;
	         case GoLeft:
   ...
   public Point Soul_Spawn()
   {
     ...
     // int result_column = -1;
      ...
      return new Point(0,0 ); //부활Spawn위치
   }
}
